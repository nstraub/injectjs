...
Ok, so one global function and one global variable aren't much to worry about, but this is just the mouth world example. With InjectJS you start by registering Mouth as a type:
...
Marvellous... (if only markdown had a descriptor for sarcasm). One good thing happened here though. You now have a mouth type that is no longer global! Next step is to use this newly created type somewhere in your code... how about a main method? you know... the one that runs when the app starts, inside `$(function() {})` or somewhere similar:
    ...
    injector.registerMain(['mouth', function (mouth) {
        mouth.greet('world');
    }
...
Now you have three choices: make Mouth global again, instantiate `Foo` and `Bar` within `Mouth`'s scope, or register these guys with InjectJS:
    ...
    injector.registerType('foo', ['mouth', function (mouth) {
        this.greet = function () {
            mouth.greet('sir');
        }
    }]);

    injector.registerType('bar', ['mouth', function (mouth) {
        this.greet = function () {
            mouth.greet('mate!');
        }
    }]);
    
    injector.registerMain(['foo', 'bar', function(foo, bar) {
...
Let's ignore the Liskov Substitution Principle violation's in Foo for a while... what's wrong with this code?
...
Fourth file (party.js):

    $('#party').on('start', injector.inject(['foo', 'bar', function () {
        foo.startParty();
        foo.greet();
        bar.greet();
        foo.endParty();
...
Now, we have four, decoupled units of logic that work fine together without really even knowing about each other. We could even split the people.js file into foo.js and bar.js and the code would still work fine, because it's now the injector's responsibility to manage availability of the resources. So let's see:

1. It's loosely coupled.
2. Foo and Bar are singletons (bonus points if you noticed on your own). When registering a type, it's default lifetime is `transient`, which means a new instance of the type is created for every place it is injected into. by specifying `'singleton'` as the third argument, we change the objects behaviour to one instance per application, as simple as that (of course you can make a global variable and clone the singleton and muck everything up by adding an explode method that brings the party to shambles, but that would be kind of defeating the purpose, and can be done with the injector anyways :P)
...
- 0.4 RequireJS and Grunt build process. Code decoupling and file restructuring
- 0.5 method injection (currying).
...
**example**
    ...
    injector.registerType(
      ...
      ['my-test-type', function (message) { 
        this.print = function () { 
          console.log('mouth, ' + message.mouth) 
        }
...
- provider (function|array): mandatory. 
  When provider is a function, its the actual provider being registered and is said not to have any dependencies of its own. 
...
**parameters**
- name(string|array|function): mandatory. If string, the name of the type you want to instantiate. If function, the function you wish to instantiate. If array, an array of dependencies followed by the function you wish to instantiate. If you supply a name or a function, it will be instantiated as a provider, calling it directly, without the new keyword.

**example**

    var logger = injector.instantiate('message-logger');
    logger.print(); // logs 'mouth, world' to the console
...
**examples**
    ...
    first_logger.print(); // logs 'mouth, world' to the console
    second_logger.print(); // also logs 'mouth, world' to the console
    
    $('#some-element').click(injector.inject(['message-logger', function (logger) {
...
Jasmine test:
    ...
    describe 'you can inject dependencies here and not worry about setup and teardown', () ->
        beforeEach injector.inject(['logger', function (logger) {
            logger.log('starting test')
        }]);
        
        afterEach injector.inject(['logger', function (logger) {
...
**example**
    ...
    describe('running tests with a harness', function () {
        ...
        describe('using inject'), injector.inject(['logger', function (logger) {
        ...
        describe('using harness'), injector.harness(['logger', function (logger) {
